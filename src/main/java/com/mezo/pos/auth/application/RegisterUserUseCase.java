...
public class RegisterUserUseCase {
    ...
    public Map<String, Object> execute(String emailStr, String password) {
        ...
        var existingUser = userRepository.findByEmail(email.getValue());
        if (existingUser.isPresent()) {
            ...
            if (!existing.isEmailVerified()) {
                ...
                return Map.of(
                        "message", "OTP reenviado a " + email.getValue(),
                        "userId", existing.getId(),
                        "pendingVerification", true
                );
            }
            throw new DomainException("Este correo ya tiene una cuenta. Inicia sesión.");
        }
    ...
    private void validatePassword(String password) {
        ...
        if (!password.matches(".*[A-Z].*")) {
            throw new DomainException("La contraseña debe tener al menos una letra mayúscula");
        }
