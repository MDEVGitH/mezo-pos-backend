...
public class SaleController {
    ...
    public ResponseEntity<List<SaleResponse>> closeSale(
            @PathVariable UUID businessId,
            @Valid @RequestBody CloseSaleRequest request) {
        UUID closedBy = UUID.fromString((String) SecurityContextHolder.getContext().getAuthentication().getPrincipal());
        List<SaleResponse> response = closeSaleUseCase.execute(request, businessId, closedBy);
